...
func (x *XWindowDetector) ActiveWindow(ctx context.Context) (WindowInfo, error) {
	// Get window ID.
	idOut, err := exec.CommandContext(ctx, x.xdotoolPath, "getactivewindow").Output() //nolint:gosec // path validated by LookPath
	if err != nil {
	...
	// Get window title.
	titleOut, err := exec.CommandContext(ctx, x.xdotoolPath, "getactivewindow", "getwindowname").Output() //nolint:gosec // path validated by LookPath
	if err != nil {
	...
	classOut, err := exec.CommandContext(ctx, x.xpropPath, "-id", windowID, "WM_CLASS").Output() //nolint:gosec // paths validated by LookPath
	appName := unknownApp
	if err == nil {
